...
const CandidateDashboard = () => {
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 bg-gradient-to-br from-slate-50 to-slate-100">
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
          <div className="flex justify-between items-center h-16 sm:h-20">
            ...
            <div className="hidden lg:flex items-center space-x-1 flex-1 justify-end">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all shadow-md hover:shadow-lg ml-2"
              >
          ...
          {mobileMenuOpen && (
            <div className={`lg:hidden py-4 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              ...
              <a href="#courses" className={`block px-4 py-3 ${darkMode ? 'text-gray-300 hover:bg-gray-700' : 'hover:bg-blue-50'}`}>Courses</a>
              <a href="#internships" className={`block px-4 py-3 ${darkMode ? 'text-gray-300 hover:bg-gray-700' : 'hover:bg-blue-50'}`}>Internships</a>
              <button
                onClick={() => {
              ...
              <div className="px-4 py-2 flex flex-col gap-2 mt-2">
                ...
                <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg">
                  <LogOut className="w-5 h-5" />
                  Logout
                </button>
              </div>
                  ...
                  <div className="space-y-2">
                    {searchResults.courses.map((course, idx) => (
                      <div
                        ...
                        onClick={() => { navigate('/courses'); clearSearch(); }}
                  ...
                  <div className="space-y-2">
                    {searchResults.internships.map((internship, idx) => (
                      <div
                        ...
                        onClick={() => { navigate('/internships'); clearSearch(); }}
      ...
      {/* Hero Section */}
      <section id="home" className={`pt-20 min-h-screen flex items-center ${darkMode ? 'bg-gray-800' : 'bg-gradient-to-br from-blue-50 to-purple-50'}`}>
              ...
              {heroSlides.map((slide, idx) => (
                <div key={idx} className={`transition-all duration-700 ${idx === currentSlide ? 'opacity-100' : 'opacity-0 absolute'}`}>
                  ...
                  <div className="flex gap-4">
                    <a href="#courses" className="px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg font-semibold hover:shadow-xl transform hover:-translate-y-1 transition-all">
                      Browse Courses
                    </a>
                    <a href="#internships" className={`px-8 py-4 border-2 border-blue-600 text-blue-600 rounded-lg font-semibold ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-blue-50'} transition-all`}>
                      View Internships
                    </a>
                  </div>
                </div>
      ...
      {/* Courses */}
      <section id="courses" className={`py-20 ${darkMode ? 'bg-gray-800' : 'bg-gradient-to-br from-slate-50 to-slate-100'}`}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-4xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Available Courses</h2>
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-col gap-2">
                    {course._id && (() => {
                      const enrollBtn = getEnrollmentButton(course._id);
                      return (
                        <>
                          <div className="flex gap-2">
                            <button 
                              onClick={enrollBtn.action}
                              ...
                              className={`flex-1 px-4 py-3 bg-gradient-to-r ${
                                ...
                                enrollBtn.disabled ? 'opacity-50 cursor-not-allowed' : ''
                              }`}
                            >
                              {enrollBtn.text}
                            </button>
                            <button onClick={() => navigate(`/courses/${course._id}`)} className={`flex-1 px-4 py-3 border-2 border-blue-600 text-blue-600 rounded-lg font-semibold transition-all ${darkMode ? 'hover:bg-gray-800' : 'hover:bg-blue-50'}`}>
                              View Details
                            </button>
                          </div>
                          <button 
                            ...
                            disabled={enrollments[course._id] && (enrollments[course._id].status === 'pending' || enrollments[course._id].status === 'accepted')}
                            className={`w-full px-4 py-2 rounded-lg font-semibold transition-all flex items-center justify-center gap-2 ${
                              enrollments[course._id] && (enrollments[course._id].status === 'pending' || enrollments[course._id].status === 'accepted')
                          ...
                          >
                            ...
                            {enrollments[course._id] && (enrollments[course._id].status === 'pending' || enrollments[course._id].status === 'accepted')
                              ? 'Already Enrolled'
                              : isInCart(course._id, 'course') ? 'Remove from Cart' : 'Add to Cart'}
                          </button>
                        </>
                      );
                    })()}
      ...
      {/* Internships */}
      <section id="internships" className={`py-20 ${darkMode ? 'bg-gray-800' : 'bg-gradient-to-br from-slate-50 to-slate-100'}`}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-center mb-4">
          ...
          <div className="relative group">
            ...
            <div className="flex gap-8 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {internships.slice(0, 5).map((internship, idx) => (
              <div 
                key={internship._id || `internship-${idx}`} 
                className={`flex-shrink-0 w-80 snap-start rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transform hover:-translate-y-2 transition-all ${darkMode ? 'bg-gray-900' : 'bg-white'}`}
              >
                {internship.image ? (
                        ...
                        <div className="flex gap-2">
                          <button 
                            ...
                            className={`flex-1 px-4 py-3 rounded-lg font-semibold transition-all ${
                              applicationStatuses[internship._id]?.status === 'accepted'
                                ? 'bg-green-500 text-white cursor-default'
                                : applicationStatuses[internship._id]?.status === 'pending'
                                ? 'bg-yellow-500 text-white cursor-default'
                                : applicationStatuses[internship._id]?.status === 'rejected'
      ...
      {/* Testimonials */}
      <section className={`py-20 ${darkMode ? 'bg-gray-800' : 'bg-gradient-to-br from-slate-50 to-slate-100'}`}>
        <div className="max-w-7xl mx-auto px-6">
      ...
      <section id="contact" className={`py-20 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
        <div className="max-w-2xl mx-auto px-6">
          <div className={`p-8 rounded-xl shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-white border'}`}>
