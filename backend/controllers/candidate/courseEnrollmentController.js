...
exports.createEnrollment = asyncHandler(async (req, res) => {
  ...
  if (existingEnrollment) {
    ...
    if (existingEnrollment.status === 'rejected') {
      ...
      existingEnrollment.courseStartDate = undefined;
      ...
      setImmediate(async () => {
        try {
          ...
          if (managers.length > 0) {
            emailPromises.push(
              sendEnrollmentRequestToManager(managers[0].email, req.user.name, req.user.email, course.title, savedEnrollment._id)
            );
          }
          await Promise.allSettled(emailPromises);
        } catch (error) {
          console.error('Error sending enrollment emails:', error.message);
        }
      });
